...
class RecurrenceRule2 extends BaseDtoLad
{
    /**
     * Use faker to populate new RecurrenceRule, same as RecurrenceRule replacing addMethods by setMethods
     *
    ...
    public static function load() : Dto
    {
        ...
        if( ! in_array( $freq, [ 'daily', 'monthly', 'weekly' ] )) {
            ...
            sort( $list );
            $dto->setByYearDay( $list );
        }

        $dto->setByWeekNo( $weekNos );
        ...
        sort( $list );
        $dto->setByMinute( $list );

        $list = $faker->randomElements( range( '1', '59' ), 3 );
        sort( $list );
        $dto->setBySecond( $list );

        $list = $faker->randomElements( [ -3, -2, -1, 1, 2, 3 ], 2 );
        sort( $list );
        $dto->setBySetPosition( $list );
